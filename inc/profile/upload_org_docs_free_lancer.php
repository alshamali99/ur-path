...
$doc_num=strip_tags($_POST['free_lancer_num']);

$allowedExtensions = ['jpg', 'jpeg', 'png', 'gif', 'pdf'];
$maxFileSize = 20 * 1024 * 1024; // 20MB

if (isset($_FILES['upload_free_lancer'])) {
    $file = $_FILES['upload_free_lancer'];
    ...
    */
    if (move_uploaded_file($fileTmpName, $fileDestination)) {
        ...
        mysqli_query($conn,"DELETE from org_docs where u_id=".$user_l['id']." AND doc_type='free_lancer' ") or die("Er-doc-del");
        $work_file=strip_tags($_POST['upload_free_lancer']);
